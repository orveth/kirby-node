...
///
/// REUSE-bounded: `hibernate::shamir::MasterSeed` is private (only `split_seed`/`derive_subkeys`
/// touch the raw master), so this sub-derives from the PUBLIC `wallet_seed` subkey rather than
/// the master — the audited hibernate module stays UNTOUCHED. Deterministic ⇒ reconstructible:
/// the same keyring-reconstructed wallet seed always yields the same event key.
