...
async fn persistent_wallet_reopen_after_drop_is_spendable() {
    ...
    let (w, _counter_db) =
        open_persistent_wallet(&mint_url, &db_path, seed, std::collections::HashMap::new(), true, true, true, false)
            .await
    ...
    let (w2, _counter_db) =
        open_persistent_wallet(&mint_url, &db_path, seed2, std::collections::HashMap::new(), true, true, true, false)
            .await
